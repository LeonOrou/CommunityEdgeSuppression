...
def _train_epoch(model, dataset, config, optimizer, all_train_users, all_train_items,
                 train_indices, user_positive_items, batch_size, device):
...
def train_model(dataset, model, config, stage='cv', fold_num=None, verbose=True):
    ...
    return trained_model, dataset.complete_edge_index, dataset.complete_edge_weight
