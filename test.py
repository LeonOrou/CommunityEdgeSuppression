...
def plot_community_dropout_strength_impact(df, output_dir):
    ...
    metric_cols = [col for col in df.columns if col not in ['run_name', 'trial', 'model',
                                                            ...
                                                            'community_dropout_strength', 'params']]
    ...
    for metric in metric_cols:
        ...
        strength_impact = df.groupby('community_dropout_strength')[metric].mean().reset_index()
...
def visualize_wandb_metrics():
    ...
    # Group runs by name and keep only the most recent from each group
    runs_by_name = {}
    for run in all_runs:
        name = run.name
        if name not in runs_by_name or run.created_at > runs_by_name[name].created_at:
            runs_by_name[name] = run

    # Use the filtered runs
    runs = list(runs_by_name.values())

    print(f"Filtered {len(all_runs)} total runs down to {len(runs)} unique runs")
